...
const buildAndScan = (env) => {
    ...
    try {
        execFileSync(process.execPath, ['node_modules/vite/bin/vite.js', 'build', '--outDir', outDir], {
            env: { ...process.env, VITE_APPWRITE_ENDPOINT: '', VITE_APPWRITE_PROJECT_ID: '', VITE_APPWRITE_DATABASE_ID: '', VITE_APPWRITE_COLLECTION_ID: '', VITE_BASE_PATH: '/3psLCCA-web/', ...env },
            stdio: 'pipe',
            timeout: 180_000,
        });
        const shell = readFileSync(path.join(outDir, 'index.html'), 'utf8');
        assert.equal(readFileSync(path.join(outDir, '404.html'), 'utf8'), shell, 'deep links must receive the same app shell');
        assert.match(shell, /\/3psLCCA-web\/assets\//, 'asset URLs must retain the deployment base path');
